    ...
    <VTTA>{
      notification: {
        ...
        show: (message, timeout = 4000) => {
          ...
          if (element) {
            elementLeft = $(element).css("left");
            elementHeight = $(element).height() || 0;
          }

          $("#vtta-notifications").css("left", elementLeft);
          $("#vtta-notifications").css("bottom", elementHeight + 2 * MARGIN);
      ...
      hint: {
        ...
        show: (message, options: VTTANotificationOptions = {}) => {
          return new Promise((resolve) => {
            ...
            if (options.element) {
              anchor = Object.assign(
                {
                  width: $(options.element).width() || 0,
                  height: $(options.element).height() || 0,
                },
                $(options.element).offset()
              );
            }
            const noteInfo = Object.assign(
              { width: $("#vtta-hints").width() || 0, height: $("#vtta-hints").height() || 0 },
              $("#vtta-hints").position()
            );
