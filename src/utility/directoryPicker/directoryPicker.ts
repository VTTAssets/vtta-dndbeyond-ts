...
class DirectoryPicker extends FilePicker {
  constructor(options = {}) {
    super(options);
    this.field = <HTMLInputElement>this.field;
  }
  ...
  _onSubmit(event) {
    ...
    (<HTMLInputElement>this.field).value = DirectoryPicker.format({
      activeSource,
      bucket,
      path,
    });
  ...
  static parse(str) {
    ...
    if (matches) {
      let source = matches[INDEX_SOURCE];
      const current = matches[INDEX_CURRENT].trim();
